...
#### 1. 快路径优化 ✅ 已完成
**当前状态**: 关键词匹配（准确率 ~75%，响应时间 1-2秒）
**问题**: 
- "标准"关键词过度拦截复杂查询
- 无法理解语义（"比较"、"分析"应该走Q&A域）

**已完成优化**:
- ✅ 删除 `policy` 规则（避免"标准"过度拦截）
- ✅ 添加复杂查询排除逻辑（"比较"、"分析"、"规划"等）
- ✅ 添加长度限制（>20字符跳过快路径）
- ✅ 精确化关键词（"航班查询"、"酒店推荐"）
...
#### 2. 规划引擎性能优化 ✅ 已完成
**问题**: 规划查询耗时 107秒（用户不可接受）

**根因分析**:
1. 串行查询3次政策（24秒） → 改为并行（8秒）✅
2. 重复加载记忆（20秒） → 传递context参数（0秒）✅
3. LLM Prompt过长（50秒） → 精简Prompt（20秒）✅

**已完成优化**:
- ✅ 政策查询并行化（ThreadPoolExecutor）
- ✅ 避免重复加载记忆（通过参数传递context）
- ✅ 精简LLM Prompt（从2000 tokens降至500 tokens）

**预期效果**: 107秒 → **25-35秒**（↓ 70%）

#### 3. 向量库持久化加载 ✅ 已完成
**问题**: 每次调用 SearchPolicyTool 重新创建向量库（30秒）

**已完成优化**:
- ✅ 优先加载持久化向量库（`src/data/vectorstore/`）
- ✅ 全局缓存检索器（`_cached_retriever`）
- ✅ 首次加载 <1秒，后续调用 <0.01秒

**效果**: 政策查询从 32秒 降至 **7-8秒**

---

### 🟡 P1 - 快路径语义匹配（待实施）

**目标**: 从关键词匹配升级到语义匹配，准确率从 75% 提升至 95%+

**实施方案**:
1. **短期（1周内）**: 前端添加"快速查询"选项，用户手动标识
2. **中期（2周内）**: 集成通用向量模型验证可行性
...
async def warmup():
    ...
    tools = get_all_tools()
    # 触发初始化
    tools["search_policy"].invoke({"query": "测试"})
```
