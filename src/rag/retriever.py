"""
向量检索模块 - 支持DashScope API和微调本地模型

Embedding模型选择:
- cloud: DashScope text-embedding-v2 (默认)
- local_finetuned: bge-large-zh-travel-finetuned (微调模型)
"""
...
def create_vectorstore(
    ...
    FAISS vs 其他向量数据库
    - FAISSFacebook开源本地运行速度快
    - Chroma更现代支持持久化
    - Pinecone云服务适合生产环境
    - Milvus分布式适合大规模

    对比Spring AI
    - Spring AI用SimpleVectorStore内存
    - LangChain的FAISS更强大支持多种索引算法
    ...
    Returns:
        FAISS向量存储实例
    """
    print(f" 创建向量存储 (embedding_type={embedding_type})...")

    if embedding_type == "cloud":
        ...
        print("   使用DashScope API: text-embedding-v2")
    
    elif embedding_type == "local_finetuned":
...
def get_retriever(vectorstore, k=5, score_threshold=None):
    ...
    参数说明
    - k: 返回最相似的k个文档Top-K检索
    - score_threshold: 相似度阈值低于此值的文档会被过滤
    ...
    # 如果设置了阈值只返回相似度高于阈值的文档
    if score_threshold is not None:
        search_kwargs["score_threshold"] = score_threshold

    print(f" 创建检索器Top-K={k}")
...
第一章 住宿标准
1. 一线城市北京、上海、广州、深圳标准间不超过500元/晚
2. 二线城市杭州、成都、武汉等标准间不超过400元/晚
3. 三线及以下城市标准间不超过300元/晚

第二章 交通标准
1. 市内交通实报实销需提供发票
2. 城际交通距离<500公里用高铁二等座距离500公里用飞机经济舱
    """
    ...
    try:
        ...
        print("2. 是否已安装faiss-cpupip install faiss-cpu")
